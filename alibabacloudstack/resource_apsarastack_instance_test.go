...
func testSweepInstances(region string) error {
	...
	for _, v := range insts {
		...
		if v.DeletionProtection {
			...
			if err != nil {
				log.Printf("[ERROR] %#v", WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), AlibabacloudStackSdkGoERROR))
				continue
			}
		}
...
resource "alibabacloudstack_security_group_rule" "default" {
   ...
   port_range = "22/22"
   priority = 1
   security_group_id = "${element(alibabacloudstack_security_group.default.*.id,count.index)}"
