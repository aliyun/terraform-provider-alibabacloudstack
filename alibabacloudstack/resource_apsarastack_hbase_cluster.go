...
func checkParams(request *hbase.CreateClusterRequest) error {
	if request.Engine == "bds" && request.VSwitchId == "" {
		return errmsgs.WrapError(errmsgs.Error("bds is not support classic"))
	}
	return nil
}
...
func buildHBaseCreateRequest(d *schema.ResourceData, meta interface{}) (*hbase.CreateClusterRequest, error) {
	...
	if vswitchId != "" {
		...
		if request.ZoneId == "" {
			...
			return nil, errmsgs.WrapError(errmsgs.Error("The specified vswitch %s isn't in the zone %s.", vsw.VSwitchId, request.ZoneId))
		}
	...
	if d.Get("cold_storage_size").(int) < 0 {
		return nil, errmsgs.WrapError(errmsgs.Error("cold_storage_size=%s is invalid", d.Get("cold_storage_size")))
	}
...
func resourceAlibabacloudStackHBaseInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AlibabacloudStackClient)
	...
	bresponse, ok := raw.(*hbase.CreateClusterResponse)
	if err != nil {
		errmsg := ""
		if ok {
			errmsg = errmsgs.GetBaseResponseErrorMessage(bresponse.BaseResponse)
		}
		return errmsgs.WrapErrorf(err, errmsgs.RequestV1ErrorMsg, "alibabacloudstack_hbase_instance", request.GetActionName(), errmsgs.AlibabacloudStackSdkGoERROR, errmsg)
	}
...
func resourceAlibabacloudStackHBaseInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	bresponse, ok := raw.(*hbase.DeleteInstanceResponse)
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
