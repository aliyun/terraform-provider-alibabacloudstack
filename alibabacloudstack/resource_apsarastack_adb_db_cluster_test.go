...
func testSweepAdbDbInstances(region string) error {
	...
	for {
		...
		if err != nil {
			log.Println(WrapErrorf(err, DataDefaultErrorMsg, "AlibabacloudStack_adb_db_clusters", action, AlibabacloudStackSdkGoERROR))
			break
		}
		...
		for _, v := range result {
			...
			err = resource.Retry(5*time.Minute, func() *resource.RetryError {
				_, err = conn.DoRequest(StringPointer(action), nil, StringPointer("POST"), StringPointer("2019-03-15"), StringPointer("AK"), nil, request, &util.RuntimeOptions{IgnoreSSL: tea.Bool(client.Config.Insecure)})
				if err != nil {
					if NeedRetry(err) {
						wait()
