...
func TestAccAlibabacloudStackEhpcJobTemplate_basic0(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alibabacloudstack_ehpc_job_template.default"
	ra := resourceAttrInit(resourceId, AlibabacloudStackEhpcJobTemplateMap0)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &EhpcService{testAccProvider.Meta().(*connectivity.AlibabacloudStackClient)}
	}, "DescribeEhpcJobTemplate")
	rac := resourceAttrCheckInit(rc, ra)
	testAccCheck := rac.resourceAttrMapUpdateSet()
	rand := getAccTestRandInt(10000, 99999)
	name := fmt.Sprintf("tf-testacc%sehpcjobtemplate%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlibabacloudStackEhpcJobTemplateBasicDependence0)
...
var AlibabacloudStackEhpcJobTemplateMap0 = map[string]string{
	"command_line":      "./LammpsTest/lammps.pbs",
	"job_template_name": "JobTemplateName",
}

func AlibabacloudStackEhpcJobTemplateBasicDependence0(name string) string {
	return fmt.Sprintf(` 
variable "name" {
  default = "%s"
}
