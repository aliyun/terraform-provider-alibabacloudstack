...
func dataSourceAlibabacloudStackInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alibabacloudstack_instances", request.GetActionName(), AlibabacloudStackSdkGoERROR)
		}
...
func getInstanceDisksMappings(instanceMap map[string]string, meta interface{}) (map[string][]map[string]interface{}, error) {
	...
	for {
		...
		if err != nil {
			return instanceDisks, WrapErrorf(err, DataDefaultErrorMsg, "alibabacloudstack_instances", request.GetActionName(), AlibabacloudStackSdkGoERROR)
		}
		addDebug(request.GetActionName(), raw, request.RpcRequest, request)
