...
func TestAccAlibabacloudStackCenTransitMulticastDomainSourcePeer(t *testing.T) {
	var v *CbnTransitRouterMulticastGroupData
	...
	rand := getAccTestRandInt(10000, 99999)
	name := fmt.Sprintf("tf-testaccdomain_source%d", rand)

	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlibabacloudTestAccCenTransitMulticastDomainSourcePeerdependence)
	ResourceTest(t, resource.TestCase{
		PreCheck: func() {
...
func AlibabacloudTestAccCenTransitMulticastDomainSourcePeerdependence(name string) string {
	return fmt.Sprintf(
		`
variable "name" {
  default = "%s"
