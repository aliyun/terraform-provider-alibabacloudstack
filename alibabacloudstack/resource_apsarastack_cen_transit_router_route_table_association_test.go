...
func TestAccAlibabacloudStackCenTransitRouterTableAssociation0(t *testing.T) {
	var v *TransitRouterRouteTableAssociationsResponse

	resourceId := "alibabacloudstack_cen_transit_router_route_table_association.default"
	ra := resourceAttrInit(resourceId, AlibabacloudTestAccCenTransitRouterTableAssociationCheckmap)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &CenService{testAccProvider.Meta().(*connectivity.AlibabacloudStackClient)}
	}, "DoCbnDescribeTransitRouterRouteTableAssociationsRequest")
	rac := resourceAttrCheckInit(rc, ra)
	testAccCheck := rac.resourceAttrMapUpdateSet()

	rand := getAccTestRandInt(10000, 99999)
	name := fmt.Sprintf("tf-testacc%srouter_table_association%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlibabacloudTestAccCenTransitRouterTableAssociationBasicdependence)
...
var AlibabacloudTestAccCenTransitRouterTableAssociationCheckmap = map[string]string{
...
func AlibabacloudTestAccCenTransitRouterTableAssociationBasicdependence(name string) string {
	return fmt.Sprintf(
		`
variable "name" {
	default = "%s"
