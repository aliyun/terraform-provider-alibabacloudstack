...
func TestAccAlibabacloudStackMongodbShardingInstanceMongosNodeAddress(t *testing.T) {
	var v map[string]interface{}

	resourceId := "alibabacloudstack_mongodb_shardinginstance_mongosnode_address.default"
	...
	name := fmt.Sprintf("tfacc-mongosnode%d", rand)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"node_id":                   "${[for mongo in local.shard_instance.mongo_list : mongo if mongo.description == \"mongo1\"][0].node_id}",
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"private_connect_string_prefix": name + "-pr",
					"private_connect_port":          3718,
					"public_connect_string_prefix":  name + "-pu",
					"public_connect_port":           3817,
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{}),
				),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"public_connect_string_prefix":  REMOVEKEY,
					}),
				),
			},
		},
