...
func TestAccAlibabacloudStackDnsLinesDataSource(t *testing.T) {
	rand := getAccTestRandInt(10000, 99999)
	resourceId := "data.alibabacloudstack_dns_lines.default"
	...
	dsa.dataSourceTestCheck(t, rand,
		dataSourceTestAccConfig{
			existConfig: AlibabacloudTestAccDnsLinesConfigDependence(rand, map[string]string{
				"name_regex": `alibabacloudstack_dns_line.default.name`,
			}),
			fakeConfig: AlibabacloudTestAccDnsLinesConfigDependence(rand, map[string]string{
				"name_regex": `"^fake-name$"`,
			}),
		},
		dataSourceTestAccConfig{
			existConfig: AlibabacloudTestAccDnsLinesConfigDependence(rand, map[string]string{
				"ids": `[alibabacloudstack_dns_line.default.id]`,
			}),
			fakeConfig: AlibabacloudTestAccDnsLinesConfigDependence(rand, map[string]string{
				"ids": `["nonexistent-id"]`,
			}),
...
func AlibabacloudTestAccDnsLinesConfigDependence(rand int, attrMap map[string]string) string {
	var pairs []string
