...
func TestUnitAccAlibabacloudStackEDASNamespace(t *testing.T) {
	...
	dInit, _ := schema.InternalMap(p["alibabacloudstack_edas_namespace"].Schema).Data(nil, nil)
...
// 		"debug_enable":         false,
	...
	notFoundResponseMock := func(errorCode string) (map[string]interface{}, error) {
		return nil, errmsgs.GetNotFoundErrorFromString(errmsgs.GetNotFoundMessage("alibabacloudstack_edas_namespace", errorCode))
	}
	...
	// Create
	patches := gomonkey.ApplyMethod(reflect.TypeOf(&connectivity.AlibabacloudStackClient{}), "NewEdasClient", func(_ *connectivity.AlibabacloudStackClient) (*roa.Client, error) {
		return nil, &tea.SDKError{
	...
	err = resourceAlibabacloudStackEdasNamespaceCreate(dInit, rawClient)
	patches.Reset()
	assert.NotNil(t, err)
	...
	for index, errorCode := range errorCodes {
		...
		default:
			assert.Nil(t, err)
			dCompare, _ := schema.InternalMap(p["alibabacloudstack_edas_namespace"].Schema).Data(dInit.State(), nil)
			for key, value := range attributes {
	...
	err = resourceAlibabacloudStackEdasNamespaceUpdate(dExisted, rawClient)
	patches.Reset()
	assert.NotNil(t, err)
...
// 		"debug_enable":   true,
	...
	diff, err := newInstanceDiff("alibabacloudstack_edas_namespace", attributes, attributesDiff, dInit.State())
	if err != nil {
		t.Error(err)
	}
	dExisted, _ = schema.InternalMap(p["alibabacloudstack_edas_namespace"].Schema).Data(dInit.State(), diff)
	ReadMockResponseDiff = map[string]interface{}{
		"body": map[string]interface{}{
			"UserDefineRegionList": map[string]interface{}{
				"UserDefineRegionEntity": []interface{}{
	...
	for index, errorCode := range errorCodes {
		...
		default:
			assert.Nil(t, err)
			dCompare, _ := schema.InternalMap(p["alibabacloudstack_edas_namespace"].Schema).Data(dExisted.State(), nil)
			for key, value := range attributes {
				_ = dCompare.Set(key, value)
			}
	...
	for index, errorCode := range errorCodes {
		...
		err := resourceAlibabacloudStackEdasNamespaceRead(dExisted, rawClient)
		patches.Reset()
		switch errorCode {
		case "NonRetryableError":
			assert.NotNil(t, err)
	...
	err = resourceAlibabacloudStackEdasNamespaceDelete(dExisted, rawClient)
	patches.Reset()
	assert.NotNil(t, err)
	attributesDiff = map[string]interface{}{}
	diff, err = newInstanceDiff("alibabacloudstack_edas_namespace", attributes, attributesDiff, dInit.State())
