...
func resourceAlibabacloudStackNetworkAclCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alibabacloudstack_network_acl", action, AlibabacloudStackSdkGoERROR)
	}
...
func resourceAlibabacloudStackNetworkAclUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AlibabacloudStackClient)
	vpcService := VpcService{client}
	var response map[string]interface{}
	d.Partial(true)
	...
	updateNetworkAclEntriesReq["Product"] = "Vpc"
	updateNetworkAclEntriesReq["OrganizationId"] = client.Department
	...
	if d.HasChange("resources") {
		...
		if added.Len() > 0 {
			...
			associatenetworkaclrequest["Product"] = "Vpc"
			associatenetworkaclrequest["OrganizationId"] = client.Department
		...
		if removed.Len() > 0 {
			...
			unassociatenetworkaclrequest["Product"] = "Vpc"
			unassociatenetworkaclrequest["OrganizationId"] = client.Department
			resourcesMaps := make([]map[string]interface{}, 0)
