...
func (s *DnsService) DescribeGlobalLine(id string) (map[string]interface{}, error) {
	query := map[string]interface{}{
		"PageNumber": 1,
		"PageSize":   100,
	}

	resp, err := s.client.DoTeaRequest("POST", "CloudDns", "2021-06-24", "DescribeGlobalLines", "", nil, nil, query)
	if err != nil {
		return nil, err
	}
	...
	if !ok || data == nil {
		return nil, errmsgs.WrapError(errmsgs.GetNotFoundErrorFromString(fmt.Sprintf("Resource DnsLine:%s not found", id)))
	}
	for _, item := range data {
...
func (s *DnsService) DescribeDnsGtmAccessStrategy(id string) (map[string]interface{}, error) {
	...
	response, err := s.client.DoTeaRequest("POST", "CloudDns", "2021-06-24", "DescribeDnsGtmAccessStrategies", "", nil, reqQuery, nil)
	if err != nil {
		return nil, err
	}
	...
	if data, ok := response["Data"].([]interface{}); ok {
		for _, item := range data {
			if strategy, ok := item.(map[string]interface{}); ok {
				if strategy["Id"].(string) == params[1] {
					return strategy, nil
				}
			}
		}
	...
	return nil, errmsgs.GetNotFoundErrorFromString(fmt.Sprintf("DnsGtmAccessStrategy %s was not found", id))
}
