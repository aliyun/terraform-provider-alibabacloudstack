...
func TestAccApsaraStackVpnCgwsDataSourceBasic(t *testing.T) {
	rand := acctest.RandInt()
	idsConf := dataSourceTestAccConfig{
		existConfig: testAccCheckApsaraStackVpnCustomerGatewaysConfig(rand, map[string]string{
			"ids": `[ "${apsarastack_vpn_customer_gateway.default.id}" ]`,
		}),
		fakeConfig: testAccCheckApsaraStackVpnCustomerGatewaysConfig(rand, map[string]string{
			"ids": `[ "${apsarastack_vpn_customer_gateway.default.id}_fake" ]`,
		}),
	}

	nameRegexConf := dataSourceTestAccConfig{
		existConfig: testAccCheckApsaraStackVpnCustomerGatewaysConfig(rand, map[string]string{
			"name_regex": `"${apsarastack_vpn_customer_gateway.default.name}"`,
		}),
		fakeConfig: testAccCheckApsaraStackVpnCustomerGatewaysConfig(rand, map[string]string{
			"name_regex": `"${apsarastack_vpn_customer_gateway.default.name}_fake"`,
		}),
	}
...
func testAccCheckApsaraStackVpnCustomerGatewaysConfig(rand int, attrMap map[string]string) string {
	var pairs []string
...
data "apsarastack_vpn_customer_gateways" "default" {
	%s
}
...
var vpnCustomerGatewaysCheckInfo = dataSourceAttr{
	resourceId:   "data.apsarastack_vpn_customer_gateways.default",
