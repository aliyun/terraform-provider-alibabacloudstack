...
func init() {
	resource.AddTestSweepers("apsarastack_vpn_customer_gateway", &resource.Sweeper{
		Name: "apsarastack_vpn_customer_gateway",
...
func testAccCheckVpnCustomerGatewayDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "apsarastack_vpn_customer_gateway" {
			continue
		}
...
func TestAccApsaraStackVpnCustomerGatewayBasic(t *testing.T) {
	var v vpc.DescribeCustomerGatewayResponse

	resourceId := "apsarastack_vpn_customer_gateway.default"
...
func TestAccApsaraStackVpnCustomerGatewayMulti(t *testing.T) {
	var v vpc.DescribeCustomerGatewayResponse

	resourceId := "apsarastack_vpn_customer_gateway.default.4"
	ra := resourceAttrInit(resourceId, nil)
	serviceFunc := func() interface{} {
		return &VpnGatewayService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}
