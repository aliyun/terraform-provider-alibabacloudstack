...
func TestAccApsaraStackDatahubSubscription_basic(t *testing.T) {
	var v *datahub.GetSubscriptionResult

	resourceId := "apsarastack_datahub_subscription.default"
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					"project_name": "${apsarastack_datahub_project.basic.name}",
					"topic_name":   "${apsarastack_datahub_topic.basic.name}",
				}),
...
func TestAccApsaraStackDatahubSubscription_multi(t *testing.T) {
	var v *datahub.GetSubscriptionResult

	resourceId := "apsarastack_datahub_subscription.default.4"
	ra := resourceAttrInit(resourceId, datahubSubscriptionBasicMap)
...
func resourceDatahubSubscriptionConfigDependence(name string) string {
	...
	resource "apsarastack_datahub_project" "basic" {
	  name = "${var.project_name}"
	  comment = "project for basic."
	}
	resource "apsarastack_datahub_topic" "basic" {
	  project_name = "${apsarastack_datahub_project.basic.name}"
	  name = "${var.topic_name}"
