...
func TestAccApsaraStackAdbDbClustersDataSource(t *testing.T) {
	rand := acctest.RandInt()
	nameConf := dataSourceTestAccConfig{
		existConfig: testAccCheckApsaraStackAdbDbClusterDataSourceConfig(rand, map[string]string{
			"description_regex": `"${apsarastack_adb_db_cluster.default.description}"`,
		}),
		fakeConfig: testAccCheckApsaraStackAdbDbClusterDataSourceConfig(rand, map[string]string{
	...
	var AdbClusterCheckInfo = dataSourceAttr{
		resourceId:   "data.apsarastack_adb_db_clusters.default",
...
func testAccCheckApsaraStackAdbDbClusterDataSourceConfig(rand int, attrMap map[string]string) string {
	var pairs []string
...
data "apsarastack_adb_db_clusters" "default" {	
	enable_details = true
	%s
}
