...
func resourceApsaraStackDtsSubscriptionJob() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackDtsSubscriptionJobCreate,
		Read:   resourceApsaraStackDtsSubscriptionJobRead,
		Update: resourceApsaraStackDtsSubscriptionJobUpdate,
		Delete: resourceApsaraStackDtsSubscriptionJobDelete,
...
func resourceApsaraStackDtsSubscriptionJobCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_dts_subscription_job", action, ApsaraStackSdkGoERROR)
	}

	d.SetId(fmt.Sprint(response["JobId"]))
	d.Set("dts_instance_id", response["InstanceId"])

	return resourceApsaraStackDtsSubscriptionJobUpdate(d, meta)
}
func resourceApsaraStackDtsSubscriptionJobRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	dtsService := DtsService{client}
	object, err := dtsService.DescribeDtsSubscriptionJob(d.Id())
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_dts_subscription_job dtsService.DescribeDtsSubscriptionJob Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackDtsSubscriptionJobUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	dtsService := DtsService{client}
	var response map[string]interface{}
	d.Partial(true)
	...
	if update {
		...
		err = resourceApsaraStackDtsSubscriptionJobStatusFlow(d, meta, target)
		if err != nil {
	...
	d.Partial(false)
	return resourceApsaraStackDtsSubscriptionJobRead(d, meta)
}
func resourceApsaraStackDtsSubscriptionJobDelete(d *schema.ResourceData, meta interface{}) error {
...
func resourceApsaraStackDtsSubscriptionJobStatusFlow(d *schema.ResourceData, meta interface{}, target string) error {
	client := meta.(*connectivity.ApsaraStackClient)
	dtsService := DtsService{client}
	var response map[string]interface{}
	object, err := dtsService.DescribeDtsSubscriptionJob(d.Id())
