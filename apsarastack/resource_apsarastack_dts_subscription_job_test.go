...
func init() {
	resource.AddTestSweepers(
		"apsarastack_dts_subscription_job",
		&resource.Sweeper{
			Name: "apsarastack_dts_subscription_job",
...
func testSweepDtsSubscriptionJob(region string) error {
	...
	for {
		...
		if err != nil {
			log.Printf("[ERROR] Failed to fetch Dts SubscriptionJobs: %s", WrapErrorf(err, DataDefaultErrorMsg, "apsarastack_dts_subscription_jobs", action, ApsaraStackSdkGoERROR))
			return nil
		}
...
func TestAccApsaraStackDTSSubscriptionJob_basic0(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_dts_subscription_job.default"
	ra := resourceAttrInit(resourceId, ApsaraStackDTSSubscriptionJobMap0)
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackDTSSubscriptionJobBasicDependence0)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"subscription_instance_vpc_id":       "${apsarastack_vpc.default1.id}",
					"subscription_instance_vswitch_id":   "${apsarastack_vswitch.default1.id}",
				}),
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					"subscription_instance_network_type": "vpc",
					"subscription_instance_vpc_id":       "${apsarastack_vpc.default2.id}",
					"subscription_instance_vswitch_id":   "${apsarastack_vswitche.default2.id}",
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						"subscription_instance_network_type": "vpc",
...
func TestAccApsaraStackDTSSubscriptionJob_basic1(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_dts_subscription_job.default"
	ra := resourceAttrInit(resourceId, ApsaraStackDTSSubscriptionJobMap0)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
...
func TestAccApsaraStackDTSSubscriptionJob_basic2(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_dts_subscription_job.default"
	ra := resourceAttrInit(resourceId, ApsaraStackDTSSubscriptionJobMap0)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
...
var ApsaraStackDTSSubscriptionJobMap0 = map[string]string{
	"auto_pay":                         NOSET,
...
func ApsaraStackDTSSubscriptionJobBasicDependence0(name string) string {
	return fmt.Sprintf(` 
variable "name" {
...
resource "apsarastack_vpc" "default1" {
  vpc_name       = var.name
  cidr_block     = "10.0.0.0/8"
}

resource "apsarastack_vswitch" "default1" {
  vpc_id            = apsarastack_vpc.default1.id
  cidr_block        = "10.1.0.0/16"
  availability_zone           = data.apsarastack_zones.default.zones[0].id
  name      = var.name
}
resource "apsarastack_vpc" "default2" {
  vpc_name       = var.name
  cidr_block     = "172.16.0.0/16"
}

resource "apsarastack_vswitch" "default2" {
  vpc_id            = apsarastack_vpc.default2.id
  cidr_block        = "172.16.0.0/24"
  availability_zone           = data.apsarastack_zones.default.zones[0].id
  name      = var.name
}
