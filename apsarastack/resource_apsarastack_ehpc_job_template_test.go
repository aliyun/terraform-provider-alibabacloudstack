...
func TestAccApsaraStackEhpcJobTemplate_basic0(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_ehpc_job_template.default"
	ra := resourceAttrInit(resourceId, ApsaraStackEhpcJobTemplateMap0)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &EhpcService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}, "DescribeEhpcJobTemplate")
	rac := resourceAttrCheckInit(rc, ra)
	testAccCheck := rac.resourceAttrMapUpdateSet()
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tf-testacc%sehpcjobtemplate%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackEhpcJobTemplateBasicDependence0)
...
var ApsaraStackEhpcJobTemplateMap0 = map[string]string{
	"command_line":      "./LammpsTest/lammps.pbs",
	"job_template_name": "JobTemplateName",
}

func ApsaraStackEhpcJobTemplateBasicDependence0(name string) string {
	return fmt.Sprintf(` 
variable "name" {
  default = "%s"
}
