...
func TestAccApsaraStackApigatewayApisDataSource(t *testing.T) {
	rand := acctest.RandIntRange(1000000, 9999999)
	resourceId := "data.apsarastack_api_gateway_apis.default"
	...
	groupAndIdsConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"ids": []string{"${apsarastack_api_gateway_api.default.api_id}"},
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"ids": []string{"${apsarastack_api_gateway_api.default.api_id}_fake"},
		}),
	}

	nameRegexConf := dataSourceTestAccConfig{
		existConfig: testAccConfig(map[string]interface{}{
			"name_regex": "${apsarastack_api_gateway_api.default.name}",
		}),
		fakeConfig: testAccConfig(map[string]interface{}{
			"name_regex": "${apsarastack_api_gateway_api.default.name}_fake",
		}),
	}
	...
	allConf := dataSourceTestAccConfig{
		...
		fakeConfig: testAccConfig(map[string]interface{}{
			"group_id":   "${apsarastack_api_gateway_group.default.id}_fake",
			"ids":        []string{"${apsarastack_api_gateway_api.default.api_id}"},
			"name_regex": "${apsarastack_api_gateway_api.default.name}",
		}),
	}
