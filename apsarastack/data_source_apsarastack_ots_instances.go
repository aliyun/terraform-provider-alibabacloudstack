...
func dataSourceApsaraStackOtsInstances() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApsaraStackOtsInstancesRead,
...
func dataSourceApsaraStackOtsInstancesRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	otsService := OtsService{client}

	allInstanceNames, err := otsService.ListOtsInstance(PageSizeLarge, 1)
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_ots_instances", "ListOtsInstance", ApsaraStackSdkGoERROR)
	}
