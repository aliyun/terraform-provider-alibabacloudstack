...
func resourceApsaraStackOtsTable() *schema.Resource {
...
func resourceAliyunOtsTableCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(6*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_ots_table", "CreateTable", AliyunTablestoreGoSdk)
	}
...
func parseId(d *schema.ResourceData, meta interface{}) (instanceName, tableName string, err error) {
	...
	if len(split) == 1 {
		// For compatibility
		if meta.(*connectivity.ApsaraStackClient).OtsInstanceName != "" {
			tableName = split[0]
			instanceName = meta.(*connectivity.ApsaraStackClient).OtsInstanceName
			d.SetId(fmt.Sprintf("%s%s%s", instanceName, COLON_SEPARATED, tableName))
		} else {
