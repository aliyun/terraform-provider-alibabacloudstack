...
func resourceApsaraStackGpdbAccount() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackGpdbAccountCreate,
		Read:   resourceApsaraStackGpdbAccountRead,
		Update: resourceApsaraStackGpdbAccountUpdate,
		Delete: resourceApsaraStackGpdbAccountDelete,
...
func resourceApsaraStackGpdbAccountCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_gpdb_account", action, ApsaraStackSdkGoERROR)
	}
	...
	return resourceApsaraStackGpdbAccountRead(d, meta)
}
func resourceApsaraStackGpdbAccountRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	gpdbService := GpdbService{client}
	object, err := gpdbService.DescribeGpdbAccount(d.Id())
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_gpdb_account gpdbService.DescribeGpdbAccount Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackGpdbAccountUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	var response map[string]interface{}
	parts, err := ParseResourceId(d.Id(), 2)
	if err != nil {
...
func resourceApsaraStackGpdbAccountDelete(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[WARN] Cannot destroy resourceapsarastackGpdbAccount. Terraform will remove this resource from the state file, however resources may remain.")
	return nil
}
