...
const DataApsarastackVswitchZones = `
...
const DataApsarastackInstanceTypes = `
...
`
const DataApsarastackInstanceTypes_Eni2 = `
data "apsarastack_instance_types" "default" {
  availability_zone = data.apsarastack_zones.default.zones[0].id
  eni_amount        = 2
  sorted_by         = "Memory"
...
locals {
  resizeable_instance_ids = [for type_name, type in data.apsarastack_instance_types.resizeable : type.instance_types.0.id if length(type.instance_types ) > 0 ]
  resizeable_instance_id = length(local.resizeable_instance_ids ) > 0 ? local.resizeable_instance_ids[0]:  local.instance_type_id
}
`

const DataApsarastackImages = `
...
const EcsInstanceCommonNoZonesTestCase = DataApsarastackVswitchZones + DataApsarastackInstanceTypes + DataApsarastackImages + `
...
const EcsInstanceCommonTestCase = DataApsarastackVswitchZones + DataApsarastackInstanceTypes + DataApsarastackImages + `
resource "apsarastack_vpc" "default" {
  name       = "${var.name}"
  cidr_block = "172.16.0.0/16"
}
...
const SlbListenerVserverCommonTestCase = DataApsarastackVswitchZones + DataApsarastackInstanceTypes + DataApsarastackImages + `
resource "apsarastack_vpc" "default" {
  name = "${var.name}"
  cidr_block = "172.16.0.0/16"
}
