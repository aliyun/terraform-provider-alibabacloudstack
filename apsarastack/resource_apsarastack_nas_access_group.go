...
func resourceApsaraStackNasAccessGroup() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackNasAccessGroupCreate,
		Read:   resourceApsaraStackNasAccessGroupRead,
		Update: resourceApsaraStackNasAccessGroupUpdate,
		Delete: resourceApsaraStackNasAccessGroupDelete,
...
func resourceApsaraStackNasAccessGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_nas_access_group", action, ApsaraStackSdkGoERROR)
	}

	d.SetId(fmt.Sprint(response["AccessGroupName"], ":", request["FileSystemType"]))

	return resourceApsaraStackNasAccessGroupRead(d, meta)
}
func resourceApsaraStackNasAccessGroupRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	...
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource apsarastack_nas_access_group nasService.DescribeNasAccessGroup Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceApsaraStackNasAccessGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	var response map[string]interface{}
	if len(strings.Split(d.Id(), ":")) != 2 {
		d.SetId(fmt.Sprintf("%v:%v", d.Id(), "standard"))
...
func resourceApsaraStackNasAccessGroupDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	if len(strings.Split(d.Id(), ":")) != 2 {
		d.SetId(fmt.Sprintf("%v:%v", d.Id(), "standard"))
	}
