...
func init() {
	resource.AddTestSweepers("aapsarastack_hbase_instance", &resource.Sweeper{
		Name: "apsarastack_hbase_instance",
...
func ApsaraStackHbaseBasicDependence(name string) string {
	return fmt.Sprintf(`
...
func TestAccApsaraStackHBaseInstanceVpc(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_hbase_instance.default"
	ra := resourceAttrInit(resourceId, nil)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &HBaseService{testAccProvider.Meta().(*connectivity.ApsaraStackClient)}
	}, "DescribeHBaseInstance")
	rac := resourceAttrCheckInit(rc, ra)
	testAccCheck := rac.resourceAttrMapUpdateSet()
	rand := acctest.RandIntRange(1000000, 9999999)
	name := fmt.Sprintf("tf-testAcc%sVpc%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackHbaseBasicDependence)
