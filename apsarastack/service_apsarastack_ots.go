...
func (s *OtsService) ListOtsInstance(pageSize int, pageNum int) ([]string, error) {
	...
	for {
		...
		if err != nil {
			return nil, WrapErrorf(err, DefaultErrorMsg, "alicloud_ots_instances", req.GetActionName(), ApsaraStackSdkGoERROR)
		}
...
func (s *OtsService) DescribeOtsInstance(id string) (inst ots.InstanceInfo, err error) {
	...
	if err != nil {
		if NotFoundError(err) {
			return inst, WrapErrorf(err, NotFoundMsg, ApsaraStackSdkGoERROR)
		}
		return inst, WrapErrorf(err, DefaultErrorMsg, id, request.GetActionName(), ApsaraStackSdkGoERROR)
	}
...
func (s *OtsService) ListOtsInstanceVpc(id string) (inst []ots.VpcInfo, err error) {
	...
	if err != nil {
		return inst, WrapErrorf(err, DataDefaultErrorMsg, "alicloud_ots_instance_attachments", request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
	resp, _ := raw.(*ots.ListVpcInfoByInstanceResponse)
	if resp.TotalCount < 1 {
...
func (s *OtsService) DescribeOtsInstanceTypes() (types []string, err error) {
	...
	if err != nil {
		return nil, WrapErrorf(err, DefaultErrorMsg, "alicloud_ots_instance", request.GetActionName(), ApsaraStackSdkGoERROR)
	}
	addDebug(request.GetActionName(), raw, request.RpcRequest, request)
