...
func (s *OssService) HeadOssBucketObject(bucketName string, objectName string) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, objectName, "HeadObject", ApsaraStackOssGoSdk)
	}
	...
	if resp["asapiSuccess"] == false && (resp["Message"] == "Not Found" || resp["Code"] == "NoSuchKey") {
		return WrapErrorf(Error(GetNotFoundMessage("OssObject", objectName)), NotFoundMsg, ApsaraStackSdkGoERROR)
	}
