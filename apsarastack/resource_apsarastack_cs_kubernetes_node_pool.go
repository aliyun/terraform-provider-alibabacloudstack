...
const ResourceName = "resource_apsarastack_cs_kubernetes_permissions"

func resourceApsaraStackCSKubernetesNodePool() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackCSKubernetesNodePoolCreate,
		Read:   resourceApsaraStackCSNodePoolRead,
		Update: resourceApsaraStackCSNodePoolUpdate,
		Delete: resourceApsaraStackCSNodePoolDelete,
...
func resourceApsaraStackCSKubernetesNodePoolCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_kubernetes_node_pool", "PrepareKubernetesNodePoolArgs", err)
	}
	...
	if err = invoker.Run(func() error {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_kubernetes_node_pool", "CreateKubernetesNodePool", raw)
	}
	...
	if ok != true {
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_kubernetes_node_pool", "ParseKubernetesNodePoolResponse", raw)
	}
	...
	return resourceApsaraStackCSNodePoolRead(d, meta)
}

func resourceApsaraStackCSNodePoolUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	csService := CsService{client}
...
func resourceApsaraStackCSNodePoolRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	csService := CsService{client}
...
func resourceApsaraStackCSNodePoolDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	csService := CsService{client}
	invoker := NewInvoker()
...
func attachExistingInstance(d *schema.ResourceData, meta interface{}) error {
	...
	client, err := meta.(*connectivity.ApsaraStackClient).NewRoaCsClient()
	if err != nil {
