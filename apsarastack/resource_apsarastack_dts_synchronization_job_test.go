...
func TestAccApsaraStackDTSSynchronizationJob_basic0(t *testing.T) {
	var v map[string]interface{}
	resourceId := "apsarastack_dts_synchronization_job.default"
	ra := resourceAttrInit(resourceId, ApsaraStackDTSSynchronizationJobMap0)
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, ApsaraStackDTSSynchronizationJobBasicDependence0)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"source_endpoint_instance_id":        "${apsarastack_db_instance.rsinstance.id}",
					...
					"destination_endpoint_instance_id":   "${apsarastack_db_instance.dsinstance.id}",
...
var ApsaraStackDTSSynchronizationJobMap0 = map[string]string{
	"error_phone":                      NOSET,
	"error_notice":                     NOSET,
...
func ApsaraStackDTSSynchronizationJobBasicDependence0(name string) string {
	return fmt.Sprintf(` 
variable "name" {
  default = "%s"
}
...
resource "apsarastack_db_instance" "dsinstance" {
  engine           = "MySQL"
  engine_version   = "5.6"
...
resource "apsarastack_db_instance" "rsinstance" {
  engine           = "MySQL"
  engine_version   = "5.6"
  instance_type        = "rds.mysql.s2.large"
  instance_storage     = "30"
...
resource "apsarastack_db_database" "db" {
  count       = 2
  instance_id = apsarastack_db_instance.dsinstance.id
...
resource "apsarastack_db_database" "db_r" {
  count       = 2
  instance_id = apsarastack_db_instance.rsinstance.id
  name        = "tfaccountpri_${count.index}"
  description = "from terraform"
character_set =  "UTF8"
}

resource "apsarastack_db_account" "account_r" {
  instance_id      =apsarastack_db_instance.rsinstance.id
  name        = "tftestdts"
  password    = "inputYourCodeHere"
  description = "from terraform"
...
resource "apsarastack_db_account_privilege" "privilege_r" {
  instance_id  = apsarastack_db_instance.rsinstance.id
  account_name = apsarastack_db_account.account_r.name
  privilege    = "ReadWrite"
  db_names     = apsarastack_db_database.db_r.*.name
}
