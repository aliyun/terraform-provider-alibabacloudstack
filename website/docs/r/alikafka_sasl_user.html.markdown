...
* `type` - (Optional, ForceNew) The authentication mechanism. Valid values: `plain`, `scram`. Default value: `plain`.
...
* `type` - (Computed) The authentication mechanism. Valid values: `plain`, `scram`.
