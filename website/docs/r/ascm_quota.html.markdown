...
* `product_name` - (Required) The name of the service. Valid values: ECS, OSS, VPC, DRDS, RDS, SLB, ODPS, EIP, GPDB, R-KVSTORE, NAS, and DDS.
...
ASCM Quota can be imported using the ProductName, QuotaType and QuotaTypeId in format `ProductName:QuotaType:QuotaTypeId`, e.g.
