...
* `account_password` - (Optional) Password of the root account. It must contain at least three of the following: uppercase letters, lowercase letters, numbers, and special characters. The length is 8 to 32 characters.
...
* `audit_status` - (Optional) The status of the Sql audit. Valid values: `enabled`, `disabled`.
* `encryption_key` - (Optional) The ID of the custom key used for TDE encryption.
* `role_arn` - (Optional) The ARN of the RAM role that allows MongoDB to access your KMS key.
...
MongoDB instances can be imported using the instance ID, e.g.
