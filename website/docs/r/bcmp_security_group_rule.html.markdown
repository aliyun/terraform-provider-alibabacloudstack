...
* `type` - (Required, ForceNew) Type of rule, `ingress` (inbound) or `egress` (outbound).
* `ip_protocol` - (Required, ForceNew) The IP protocol type. Valid values are `tcp`, `udp`, `icmp`, `gre`, and `all`.
* `policy` - (Optional, ForceNew) The policy of the rule. Valid values are `accept` and `drop`. Default is `accept`.
* `port_range` - (Required, ForceNew) The port range. For `tcp` and `udp`, the format is `start/end` (e.g., `22/22`). For `icmp`, `gre`, and `all`, use `-1/-1`.
* `priority` - (Optional, ForceNew) The priority of the rule. Valid range: 1-100. Default is 1.
* `security_group_id` - (Required) The ID of the security group.
* `cidr_ip` - (Optional, ForceNew) The CIDR IP address. Conflicts with `source_security_group_id`.
* `source_security_group_id` - (Optional, ForceNew) The ID of the source security group. Conflicts with `cidr_ip`.
