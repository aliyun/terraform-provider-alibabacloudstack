...
* `expire_time` - (Optional) The token expiration time in milliseconds. The default value varies depending on the authentication method.
...
* `id` - The resource ID in the format of `{gwInstanceId:appId}`.
