...
page_title: "Alibabacloudstack: alibabacloudstack_slb_accesslogs"
...
# alibabacloudstack\_slb\_accesslogs

This data source provides a list of slb accesslogs in an alibabacloudstack account according to the specified filters.

## Example Usage
```
variable "name" {
  default = "tf-testAcc-7707171"
}
...
data "alibabacloudstack_slb_access_logs" "default" {
...
The following arguments are supported:
  * `ids` - (Optional) - A list of slb access log IDs to filter results.
  * `load_balancer_id` - (Optional) - Slb instance id
  * `log_store_regex` - (Optional) - A regex string to filter results by slb access log name.

## Attributes Reference

The following attributes are exported in addition to the arguments listed above:
  * `access_logs` - A list of slb access logs. Each element contains the following attributes:
    * `id` - The unique ID of the slb access log.
    * `load_balancer_id` - Slb instance id
    * `log_project` - LogProject of user SLS
    * `log_store` - LogStore of user SLS
    * `log_type` - Access Log. The default is layer7
