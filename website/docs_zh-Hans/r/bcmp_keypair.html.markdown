...
* `key_pair_name` - (选填, ForceNew) 密钥对的名称。值包含 2 到 128 个英文或中文字符。名称必须以字母或中文字符开头。不能以 `http://` 或 `https://` 开头。值可以包含数字、冒号 (`:`)、下划线 (`_`) 或连字符 (`-`)。
* `public_key` - (选填, ForceNew) 您想要导入并使用 AlibabaCloudStack 密钥对管理的现有公钥。
* `key_file` - (选填, ForceNew) 新创建的密钥对的私钥将保存的文件名。强烈建议在创建密钥对时指定此参数，因为如果不保存，您将无法在之后检索私钥。

> **注意:** 如果未设置 `key_pair_name`，Terraform 将生成唯一的 ID 来替换它。
