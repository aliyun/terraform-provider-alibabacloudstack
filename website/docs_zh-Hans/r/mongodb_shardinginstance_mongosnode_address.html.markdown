...
resource "alibabacloudstack_mongodb_shardinginstance_mongosnode_address" "default" {
...
* `public_connect_string_prefix` - (选填) 公网链接地址前缀
* `private_connect_string_prefix` - (选填) 私网链接地址前缀
* `public_connect_port` - (选填) 公网链接地址端口
* `private_connect_port` - (选填) 私网链接端口
