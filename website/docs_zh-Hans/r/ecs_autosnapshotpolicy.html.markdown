...
* `disk_ids` - (可选) 绑定策略的云盘ID列表。
* `tags` - (可选) 为实例打标签的键值对映射。
* `name` - (**已弃用**) 自 1.63.0 版本起已弃用，请使用新参数 `auto_snapshot_policy_name`。
...
* `id` - 自动快照策略的ID。
* `name` - (**已弃用**) 自动快照策略的名称。自 1.63.0 版本起已弃用，请使用 `auto_snapshot_policy_name`。
* `auto_snapshot_policy_name` - 自动快照策略的名称。
