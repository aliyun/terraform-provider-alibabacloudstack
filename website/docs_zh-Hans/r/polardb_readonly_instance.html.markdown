...
使用Provider配置的凭证在指定的资源集下编排PolarDB只读实例。
...
* `instance_type` - (必填) 数据库实例类型。
* `instance_storage` - (必填) 用户定义的数据库实例存储空间。
* `instance_name` - (可选) 数据库实例名称。长度为2到256个字符的字符串。
* `parameters` - (可选) 在数据库实例启动后需要设置的参数集合。
  * `name` - (必填) 参数名称。
  * `value` - (必填) 参数值。
...
* `engine` - 数据库类型。
* `port` - PolarDB数据库连接端口。
* `connection_string` - PolarDB数据库连接字符串。
