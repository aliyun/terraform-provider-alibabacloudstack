...
* `engine_version` - (必填，ForceNew) 数据库版本。可选值请参考最新文档 [CreateDBInstance](https://www.alibabacloud.com/help/doc-detail/26228.htm) 中的 `EngineVersion`。
* `db_instance_storage_type` - (必填，ForceNew) 实例的存储类型。有效值：`local_ssd`、`cloud_ssd`、`cloud_essd`、`cloud_essd2`、`cloud_essd3`、`cloud_pperf`、`cloud_sperf`。
* `master_db_instance_id` - (可选，ForceNew，已废弃) 主实例的ID。请使用 `master_instance_id` 替代。
* `master_instance_id` - (可选，ForceNew) 主实例的ID。
...
* `zone_id` - (可选，ForceNew) 启动数据库实例所在的可用区。
* `vswitch_id` - (可选，ForceNew) 用于在一个VPC中启动数据库实例的虚拟交换机ID。
