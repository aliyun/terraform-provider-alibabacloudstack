...
* `backup_period` - (选填，已弃用) MongoDB 实例的备份周期。当设置了 `preferred_backup_time` 时为必填项。有效值：`[Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday]`。默认值：`[Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday]`。此字段已弃用，请使用 `preferred_backup_period` 代替。
* `preferred_backup_period` - (选填) MongoDB 实例的备份周期。有效值：`[Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday]`。默认值：`[Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday]`。
* `backup_time` - (选填，已弃用) MongoDB 实例的备份时间窗口。格式为 `HH:mmZ-HH:mmZ`。时间设置间隔为一小时。如果未设置，默认返回类似 `23:00Z-24:00Z`。此字段已弃用，请使用 `preferred_backup_time` 代替。
...
* `account_password` - (选填) root 账户的密码。必须包含以下至少三种字符：大写字母、小写字母、数字和特殊字符。长度为 8 到 32 个字符。
...
* `audit_status` - (选填) Sql审计开关状态. 取值范围: `enabled`, `disabled`.
* `encryption_key` - (选填) 用于 TDE 加密的自定义密钥 ID。
* `role_arn` - (选填) 允许 MongoDB 访问您的 KMS 密钥的 RAM 角色 ARN。
...
MongoDB 实例可以使用实例 ID 导入，例如：

```
$ terraform import alibabacloudstack_mongodb_instance.example dds-bp1234567890abcdef
```