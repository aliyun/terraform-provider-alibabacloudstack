...
* `transit_router_multicast_domain_id` - (必填, 变更时强制重建) 转发路由器组播域ID。修改此参数会强制重新创建资源。
* `resource_type` - (必填, 变更时强制重建) 资源类型。取值范围：`VPC`、`Connect`。修改此参数会强制重新创建资源。
* `vswitch_id` - (可选, 变更时强制重建, Computed) 交换机ID。当 `resource_type` 为 `VPC` 时必填。修改此参数会强制重新创建资源。此属性由 API 返回，无法手动设置。
* `network_interface_id` - (可选, 变更时强制重建) 弹性网卡 ENI ID。当 `resource_type` 为 `VPC` 时必填。修改此参数会强制重新创建资源。
* `connect_peer_id` - (可选, 变更时强制重建) Connect Peer ID。当 `resource_type` 为 `Connect` 时必填。修改此参数会强制重新创建资源。
* `connect_attachment_id` - (可选, 变更时强制重建, Computed) Connect Attachment ID。当 `resource_type` 为 `Connect` 时必填。修改此参数会强制重新创建资源。此属性由 API 返回，无法手动设置。
