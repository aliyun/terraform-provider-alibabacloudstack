...
* `primary_db_instance_id` - (必填，变更后重建) 主实例的 ID。修改此参数会强制重新创建只读实例。
* `storage` - (必填，变更后重建) PolarDB-X 只读实例的存储容量，单位为 GB。
...
* `engine_version` - (选填，变更后重建) PolarDB-X 只读实例的引擎版本。取值：`5.7`、`8.0`。
