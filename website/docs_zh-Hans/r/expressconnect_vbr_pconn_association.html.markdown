...
支持以下参数：
  * `physical_connection_id` - (必填, 变更时强制重建) - 物理专线实例ID。
  * `vbr_id` - (必填, 变更时强制重建) - VBR实例ID。
  * `vlan_id` - (必填, 变更时强制重建) - VBR 的 VLAN ID。取值范围：`0` 到 `2999`。只有物理专线的所有者可以指定该参数，同一条物理专线下的两个 VBR 的 VLAN ID 不能相同。
  * `local_gateway_ip` - (选填, 变更时强制重建) - VBR实例的阿里云侧互联IP。
  * `peer_gateway_ip` - (选填, 变更时强制重建) - VBR 实例的客户侧互联 IP。该属性只允许 VBR 所有者指定或修改。为物理专线所有者创建 VBR 实例时必填。
  * `peering_subnet_mask` - (选填, 变更时强制重建) - VBR 实例的阿里云侧和客户侧互联 IP 的子网掩码。两个 IP 地址必须位于同一子网中。
  * `enable_ipv6` - (选填, 变更时强制重建) - 是否启用 IPv6。取值：`true`（启用）、`false`（默认值，关闭）。
  * `local_ipv6_gateway_ip` - (选填, 变更时强制重建) - VBR实例的阿里云侧互联IPv6地址。
  * `peer_ipv6_gateway_ip` - (选填, 变更时强制重建) - VBR 实例的客户侧互联 IPv6 地址。该属性只允许 VBR 所有者指定或修改。为物理专线所有者创建 VBR 实例时必填。注意：当 `enable_ipv6` 为 `true` 时必填，且需与 `local_ipv6_gateway_ip` 和 `peering_ipv6_subnet_mask` 同时使用。
  * `peering_ipv6_subnet_mask` - (选填, 变更时强制重建) - VBR 实例的阿里云侧和客户侧互联 IPv6 的子网掩码。两个 IPv6 地址必须位于同一子网中。注意：当 `enable_ipv6` 为 `true` 时必填，且需与 `local_ipv6_gateway_ip` 和 `peer_ipv6_gateway_ip` 同时使用。
