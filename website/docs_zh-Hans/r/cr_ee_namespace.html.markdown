...
* `instance_id` - (必填，ForceNew) 容器镜像企业版实例的 ID。更改此参数将强制创建新资源。
* `name` - (必填，ForceNew) 容器镜像企业版命名空间的名称。它可以包含 2 到 30 个字符。更改此参数将强制创建新资源。
...
* `id` - 资源 ID，格式为 `{instance_id}:{namespace}`。

## 导入

容器镜像企业版命名空间可以使用 `instance_id` 和 `name` 通过冒号连接进行导入，例如：
