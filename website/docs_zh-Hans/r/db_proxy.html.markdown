...
支持以下参数�?
  ...
  * `db_proxy_instance_status` - (选填) - 代理实例状�?
  ...
  * `effective_specific_time` - (选填) - 指定时间生效。格式：<i>yyyy-MM-dd</i>T<i>HH:mm:ss</i>Z（UTC时间）�?**EffectiveTime**�?*SpecificTime**时，该参数必传�?
  ...
  * `connection_persist` - (选填) - 连接池。取值：- **0**：关�?- **1**：开启会话级连接�?- **2**：开启事务级连接池�?
  * `causal_consist_read` - (选填) - 一致性参数。取值：- **0**：始终一�?- **1**：开启会话一致�?- **2**：开启全局一致性�?
## 属性参�?
