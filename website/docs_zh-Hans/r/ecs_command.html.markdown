...
resource "alibabacloudstack_ecs_command" "complex" {
  name              = "StopKubeletAndRemoveK8S"
  command_content   = "c3lzdGVtY3RscyBzdG9wIGt1YmVsZXQuc2VydmljZTsgc3lzdGVtY3QgcXVldWUga3ViZWxldC5zZXJ2aWNlOyBzeXN0ZW1jdGxzIGRlbW9uLXJlbG9hZDseWV -yIHJlbW92ZSBrdWJlYWRtIGt1YmVsZXQga3VidWxldGM="
...
* `name` - (必填，变更时重建) 命令的名称。它必须是唯一的，并且可以包含多达 128 个字符。
...
* `working_dir` - (可选，变更时重建) 命令将在 ECS 实例上执行的工作目录。
