...
* `connection_prefix` -(可选，支持回读) - 连接字符串的前缀。它的长度必须为1到31个字符，并且可以包含数字、字母、下划线（_）和连字符（-）。如果未指定，系统将自动生成默认值。修改此参数不会重建资源，可以在线更新。
* `port` -(可选) - 连接的端口号。默认值为`3306`，取值范围为`1000`到`65534`。
