...
此数据源提供根据指定过滤条件列出的阿里云账号下的slb accesslogs资源列表。
...
## 参数参考
以下参数是支持的：
  * `ids` - (选填) - slb 访问日志对象id的列表
  * `load_balancer_id` - (选填) - slb实例id
  * `log_store_regex` - (选填) - 用于过滤log_store得正则表达式

## Attributes Reference
除了上述参数外，还导出以下属性：
  * `access_logs` - slb 访问日志列表
    * `id` - slb 访问日志ID。
    * `load_balancer_id` - slb实例id
    * `log_project` - 用户SLS的LogProject
    * `log_store` - 用户SLS的LogStore
    * `log_type` - 访问日志，默认为layer7
