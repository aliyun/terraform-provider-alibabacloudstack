...
* `db_cluster_id` - (必需) PolarDB 集群的 ID。

## 属性参考

导出以下属性：

* `data_level1_backup_frequency` - 一级数据备份的频率。
* `data_level1_backup_period` - 一级数据备份的周期。
* `data_level1_backup_time` - 一级数据备份的时间。
* `data_level1_backup_retention_period` - 一级数据备份的保留周期。
* `data_level2_backup_retention_period` - 二级数据备份的保留周期。
* `backup_retention_policy_on_cluster_deletion` - 集群删除时的备份保留策略。
* `log_backup_retention_period` - 日志备份的保留周期。
* `preferred_backup_time` - 首选备份时间。
* `preferred_backup_period` - 首选备份周期。
* `backup_retention_period` - 备份保留周期。
* `backup_frequency` - 备份频率。
* `preferred_next_backup_time` - 下次首选备份时间。
* `data_level2_backup_period` - 二级数据备份的周期。
* `data_level2_backup_another_region_region` - 另一个区域的二级备份区域。
* `data_level2_backup_another_region_retention_period` - 另一个区域的二级备份保留周期。
* `log_backup_another_region_region` - 另一个区域的日志备份区域。
* `log_backup_another_region_retention_period` - 另一个区域的日志备份保留周期。
