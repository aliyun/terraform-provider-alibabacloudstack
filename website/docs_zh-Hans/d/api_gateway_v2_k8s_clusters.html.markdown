...
sidebar_current: "docs-Alibabacloudstack-datasource-api_gateway_v2_k8s_cluster"
description: |-
  查询API网关v2版本的K8s集群信息
---
...
查询API网关v2版本的K8s集群信息，用于获取已导入API网关的Kubernetes集群列表。
...
- **ids** (列表, 可选)：K8s集群ID列表，用于精确过滤结果。

- **k8s_cluster_name** (字符串, 可选)：K8s集群名称，用于过滤结果。

- **name_regex** (字符串, 可选)：用于过滤K8s集群名称的正则表达式。

## 属性说明
以下属性被导出：

- **id** (字符串)：K8s集群的唯一标识符，对应API返回的k8sClusterCode。

- **cluster_type** (字符串)：K8s集群的类型，对应API返回的k8sClusterType。

- **cs_cluster_id** (字符串)：容器服务集群ID，对应API返回的k8sClusterAttribute.csClusterId。

- **cs_cluster_name** (字符串)：容器服务集群名称，对应API返回的k8sClusterAttribute.csClusterName。

- **k8s_cluster_name** (字符串)：K8s集群名称，对应API返回的k8sClusterName。

- **slb_type** (字符串)：SLB类型，对应API返回的k8sClusterAttribute.slbType。

- **vpc_id** (字符串)：与K8s集群关联的VPC ID，对应API返回的k8sClusterAttribute.vpcId。