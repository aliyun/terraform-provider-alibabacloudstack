...
page_title: "Alibabacloudstack: alibabacloudstack_vpc_dhcpoptionssets"
sidebar_current: "docs-Alibabacloudstack-datasource-vpc-dhcpoptionssets"
description: |-
  提供阿里云账号下拥有的vpc dhcpoptionssets列表。
---

# alibabacloudstack\_vpc\_dhcpoptionssets
...
除了上述参数外，还导出以下属性：
  * `dhcp_options_sets` - dhcp 选项集列表。
    ...
    * `dhcp_options_set_name` - 代表资源名称的资源属性字段
    * `domain_name` - 主机名后缀
    * `domain_name_servers` - DNS服务器IP。最多传入4个DNS服务器IP，DNS服务器IP之间用半角逗号（,）隔开。
